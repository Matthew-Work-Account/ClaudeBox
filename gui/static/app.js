...
// Full teardown: dispose xterm, close SSE, and hard-kill PTY.
function _termDisconnect() {
  const _containerToKill = _termContainer;
  _termSoftDisconnect();
  if (_termXterm) {
  ...
  _termContainer = null;
  if (_containerToKill) {
    fetch("/api/containers/" + encodeURIComponent(_containerToKill) + "/terminal", { method: "DELETE" })
      .catch(function () {});
  }
}
